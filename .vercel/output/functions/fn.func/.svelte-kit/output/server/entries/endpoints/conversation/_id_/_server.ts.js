...
async function summarize(prompt) {
  ...
  return await generateFromDefaultEndpoint({
    ...
    if (!new RegExp("\\p{Emoji}", "u").test(summary.slice(0, 3))) {
      return "💬 " + summary;
    }
    return summary;
  }).catch((e) => {
...
async function preprocessMessages(messages, webSearch, multimodal, id) {
  return await Promise.all(
    structuredClone(messages).map(async (message, idx) => {
      if (idx === messages.length - 1 && webSearch && webSearch.context) {
        ...
        const previousQuestions = messages.filter((el) => el.from === "user").slice(0, -1).map((el) => el.content);
        const currentDate = format(/* @__PURE__ */ new Date(), "MMMM d, yyyy");
...
async function POST({ request, locals, params, getClientAddress }) {
  ...
  const embeddings = new OpenAIEmbeddings({
    ...
    // Make sure this version is correct and supported
    azureOpenAIApiInstanceName: "caniel-rag-chatbot-ncs",
    // This should match your Azure OpenAI instance name
    azureOpenAIApiDeploymentName: "caniel-text-embeddings-model-ada-002",
    // This should match your deployment name
    azureOpenAIBasePath: "https://caniel-rag-chatbot-ncs.openai.azure.com/openai/deployments"
    // Do not include the deployment name here
  });
