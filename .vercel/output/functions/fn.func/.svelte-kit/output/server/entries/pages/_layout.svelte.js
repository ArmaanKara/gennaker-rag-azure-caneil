import { c as create_ssr_component, b as spread, d as escape_object, a as add_attribute, f as subscribe, g as createEventDispatcher, v as validate_component, e as escape, h as each, o as onDestroy, i as set_store_value } from "../../chunks/ssr.js";
import "../../chunks/client.js";
import { n as navigating, p as page } from "../../chunks/stores.js";
import { b as base } from "../../chunks/paths.js";
import { P as PUBLIC_ORIGIN, a as PUBLIC_APP_NAME, b as PUBLIC_APP_DESCRIPTION, c as PUBLIC_APP_DISCLAIMER_MESSAGE, d as PUBLIC_APP_ASSETS } from "../../chunks/public.js";
...
import { u as useSettingsStore, c as createSettingsStore } from "../../chunks/settings2.js";
...
const MobileNav = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  title = title !== null && title !== void 0 ? title : "New Chat";
  {
    if ($navigating) {
...
const Edit = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<svg${spread(
    ...
    {}
  )}><!-- HTML_TAG_START -->${`<path fill="currentColor" d="M2 26h28v2H2zM25.4 9c.8-.8.8-2 0-2.8l-3.6-3.6c-.8-.8-2-.8-2.8 0l-15 15V24h6.4zm-5-5L24 7.6l-3 3L17.4 7zM6 22v-3.6l10-10l3.6 3.6l-10 10z"/>`}<!-- HTML_TAG_END --></svg>`;
});
const NavConversationItem = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<a data-sveltekit-noscroll href="${escape(base, true) + "/conversation/" + escape(conv.id, true)}" class="${"group flex h-10 flex-none items-center gap-1.5 rounded-lg pl-2.5 pr-2 text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 " + escape(
    ...
    true
  )}"><div class="flex flex-1 items-center truncate">${``} ${conv.avatarHash ? `<img src="${escape(base, true) + "/settings/assistants/" + escape(conv.assistantId, true) + "/avatar.jpg?hash=" + escape(conv.avatarHash, true)}" alt="Assistant avatar" class="mr-1.5 inline size-4 flex-none rounded-full object-cover"> ${escape(conv.title.replace(new RegExp("\\p{Emoji}", "gu"), ""))}` : `${conv.assistantId ? `<div class="mr-1.5 flex size-4 flex-none items-center justify-center rounded-full bg-gray-300 text-xs font-bold uppercase text-gray-500"></div> ${escape(conv.title.replace(new RegExp("\\p{Emoji}", "gu"), ""))}` : `${escape(conv.title)}`}`}</div> ${`<button type="button" class="flex h-5 w-5 items-center justify-center rounded md:hidden md:group-hover:flex" title="Edit conversation title">${validate_component(Edit, "CarbonEdit").$$render(
    $$result,
    {
...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  {
    if ($titleUpdate) {
      const convIdx = data.conversations.findIndex(({ id }) => id === ($titleUpdate === null || $titleUpdate === void 0 ? void 0 : $titleUpdate.convId));
      if (convIdx != -1) {
        data.conversations[convIdx].title = (_a = $titleUpdate === null || $titleUpdate === void 0 ? void 0 : $titleUpdate.title) !== null && _a !== void 0 ? _a : data.conversations[convIdx].title;
      }
      data.conversations = [...data.conversations];
      set_store_value(titleUpdate, $titleUpdate = null, $titleUpdate);
    }
  }
  mobileNavTitle = ["/models", "/assistants", "/privacy"].includes((_c = $page.route.id) !== null && _c !== void 0 ? _c : "") ? "" : (_d = data.conversations.find((conv) => conv.id === $page.params.id)) === null || _d === void 0 ? void 0 : _d.title;
  $$unsubscribe_page();
  $$unsubscribe_settings();
